...
quaternion_t differentiate_quat(vec3_t gyro, float a, float b, float c, float d) {
    quaternion_t dot;
    dot.a = 0.5f * (-b * gyro.x - c * gyro.y - d * gyro.z);
    dot.b = 0.5f * (a  * gyro.x + c * gyro.z - d * gyro.y);
    dot.c = 0.5f * (a  * gyro.y - b * gyro.z + d * gyro.x);
    dot.d = 0.5f * (a  * gyro.z + b * gyro.y - c * gyro.x);
    return dot;
}
...
quaternion_t madgwick_filter(vec3_t acc, vec3_t gyro, vec3_t mag) {
    ...
    quaternion_t qdot = differentiate_quat(gyro, q1, q2, q3, q4);
    qdot.a -= beta * s1;
    qdot.b -= beta * s2;
    qdot.c -= beta * s3;
    qdot.d -= beta * s4;
